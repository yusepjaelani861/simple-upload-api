...
const errorHandler = async (err: any, req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const errorCodeArray = ['PROCESS_ERROR', 'VALIDATION_ERROR', 'NOT_FOUND', 'UNAUTHORIZED'];
        if (typeof (err) !== 'undefined' && !errorCodeArray.includes(err.error?.error_code)) {
            const message = `
Netzone Media Ticket
*Error Marketplace API Backend*
===========================
...
===========================
    `
            // const response = await axios.post(`https://test.mitehost.my.id/api/20ca36496e2da44a9a2cd237548fd5dfea48d624/send`, {
            //     chatId: '-1006069817131',
            //     body: message
            // })
